...
@else
@foreach($one_video as $video)
<div class="table-responsive">
   @if($video->type == 'Link')
    
    <iframe height="450"  src="{{ url('https://www.youtube.com/embed/'.$video->video_or_link) }}" allowfullscreen="" style="width:100% !important;"></iframe>

   @endif
   @if($video->type == 'Video')
    <video height="450" controls style="width:100% !important;">
        <source src="{{url('/storage/app/activity_video/'.$video->video_or_link)}}" type="video/mp4">
    </video>
   @endif
</div>
<div class="content" style="margin-left: 0px !important;">
<h2>{{$video->activity_title}}</h2>
<p>{{$video->act_det_des}}</p>
</div>
@endforeach
@endif
</div>
...
$('.see_other_video').click(function(){
